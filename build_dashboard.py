...
<div class="page">
  ...
  <div class="tab-panel active" id="panel-story">
    ...
    <div class="hero">
      ...
      <div class="hero-stats">
        <div class="hero-stat">
          ...
          <div class="hero-desc">April 13 overnight gap — outdoor dropped to 47°F, indoor held at 77°F. The building couldn't shed heat even with a massive differential to outside.</div>
        </div>
        <div class="hero-stat">
          <div class="hero-num">81°F</div>
          <div class="hero-desc">Living room peak on an 80°F outdoor day. Indoor exceeded outdoor. The PTACs were running.</div>
        </div>
        <div class="hero-stat">
    ...
    <div class="story-section">
      ...
      <p>The problem isn't broken equipment. The equipment works. The problem is the building's thermal envelope — and no amount of PTAC maintenance changes that.</p>
    </div>

    <div class="story-section">
      <h2>The Action</h2>
      <p>Deployed Govee temperature sensors throughout the living room. Pulled 15-minute readings from <span id="s-start-date"></span> through <span id="s-end-date"></span> — <span id="s-duration"></span> days of continuous monitoring. Layered in Open-Meteo outdoor weather data at the same 15-minute cadence, aligned to the same timezone, so every indoor/outdoor comparison is apples-to-apples.</p>
    ...
    <div style="padding-top:8px">
      ...
      <div class="build-section">
        ...
        <div class="build-block">
          ...
          <p>Open-Meteo Historical Forecast API. 15-minute cadence (<code>minutely_15=temperature_2m</code>), Fahrenheit, aligned to America/New_York timezone — matching the indoor sensor cadence directly. Coordinates supplied as a build-time input only; not stored in any output.</p>
        </div>
      </div>
...
function toggleSensor(key) {
  ...
  const soloAlready = vis[key] && allKeys.every(k => vis[k] === (k === key));
  allKeys.forEach(k => {
    vis[k] = soloAlready ? true : (k === key);
...
function drawFloorPlan() {
  ...
  s += `<text x="${ptacX+8}" y="${ptacY+13}" font-family="Georgia,serif" font-size="7.5" fill="#fff" text-anchor="middle" font-weight="bold">PTAC</text>`;
  ...
  s += `<text x="${fuX+9}" y="${fuY+20}" font-family="Georgia,serif" font-size="7" fill="#fff" text-anchor="middle" font-weight="bold">AC</text>`;
  ...
  // Sensor dots
  const sensors = [
    { x:chx, y:chy, color:'#6B7C93', name:'Chris',  cell:'A5'  },
    { x:jux, y:juy, color:'#C4553A', name:'Jude',   cell:'A9'  },
    { x:edx, y:edy, color:'#D4892A', name:'Eddie',  cell:'B6'  },
    { x:hex, y:hey, color:'#5A8A6A', name:'Heath',  cell:'C2'  },
  ];

  sensors.forEach(sen => {
    s += `<circle cx="${sen.x}" cy="${sen.y}" r="8" fill="${sen.color}" opacity="0.9"/>`;
    s += `<text x="${sen.x}" y="${sen.y + 4}" font-family="Georgia,serif" font-size="8" fill="#fff" text-anchor="middle" font-weight="bold">${sen.name}</text>`;
  });
