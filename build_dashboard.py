...
#range-trigger{white-space:nowrap;text-align:left}
.cal-pop{position:absolute;top:100%;left:0;z-index:60;margin-top:6px;background:#FEFCF9;border:1px solid #D8D0C4;border-radius:6px;box-shadow:0 6px 24px rgba(0,0,0,.14);padding:12px;width:290px;max-width:calc(100vw - 32px)}
.cal-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:8px}
.cal-head span{font-size:.85rem;color:#2C2416;letter-spacing:.04em}
.cal-nav{background:none;border:1px solid #D8D0C4;border-radius:4px;width:30px;height:30px;font-size:1.1rem;line-height:1;color:#7A6A56;cursor:pointer}
.cal-nav:hover{background:#EEE8DE}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px}
.cal-dow{font-size:.66rem;color:#9A8A76;text-align:center;padding:3px 0}
.cal-day{aspect-ratio:1;display:flex;align-items:center;justify-content:center;font-size:.8rem;color:#3A3026;border-radius:4px;cursor:pointer;border:none;background:none;font-family:inherit;padding:0}
.cal-day:hover{background:#EEE8DE}
.cal-day.blank{visibility:hidden;pointer-events:none}
.cal-day.disabled{color:#D8D0C4;cursor:default;pointer-events:none;background:none}
.cal-day.in-range{background:#EDE4D3;border-radius:0}
.cal-day.sel{background:#2C2416;color:#F7F4EE;border-radius:4px}
.cal-foot{display:flex;align-items:center;justify-content:space-between;margin-top:10px}
#cal-hint{font-size:.72rem;color:#9A8A76}
.cal-full{background:none;border:1px solid #D8D0C4;border-radius:4px;padding:5px 10px;font-family:inherit;font-size:.72rem;color:#7A6A56;cursor:pointer}
.cal-full:hover{background:#EEE8DE}
@media (max-width:640px){ .cal-day{font-size:.9rem} .cal-nav{width:34px;height:34px} }
</style>
</head>
<body>
<div class="page">
  ...
  <div class="tab-panel" id="panel-data">
    ...
    <div class="controls range-row" style="gap:8px;margin-top:-6px">
      <span style="font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;color:#9A8A76">Range</span>
      <button type="button" id="range-trigger" class="date-input" onclick="toggleCal(event)">—</button>
      <div id="cal-pop" class="cal-pop" hidden>
        <div class="cal-head">
        ...
        <div class="cal-foot">
          <span id="cal-hint">Tap start date</span>
          <button type="button" class="cal-full" onclick="calFull()">Full study</button>
        </div>
      </div>
    </div>
    ...
    <div style="padding-top:8px">
      ...
      <div class="build-section">
        ...
        <div class="build-block">
          ...
          <p>Open-Meteo Historical Forecast API — 15-minute cadence, outdoor temperature and humidity, in Fahrenheit and aligned to the America/New_York timezone to match the indoor sensor cadence directly. Coordinates are supplied as a build-time input only and never stored in any output.</p>
        </div>
      </div>
...
// Default Y-axis mode by device: desktop → auto-fit, mobile → fixed (steady for comparison)
function initAxisMode() {
  const mobile = window.matchMedia('(max-width: 640px)').matches;
  autoFitY = !mobile;
  document.querySelectorAll('#axis-toggle .zbtn').forEach(b =>
    b.classList.toggle('active', b.dataset.axis === (autoFitY ? 'auto' : 'fixed')));
}
...
// ── Range calendar (single popover: tap start, then end) ────────────────────
const MIN_DATE = DATA.labels[0].slice(0, 10);
const MAX_DATE = DATA.labels[N - 1].slice(0, 10);
const CAL_MONTHS = ['January','February','March','April','May','June','July','August','September','October','November','December'];
let calY = 0, calM = 0;         // month on display
let selStart = null, selEnd = null;

function pad2(n) { return String(n).padStart(2, '0'); }
function ymd(y, m, d) { return `${y}-${pad2(m + 1)}-${pad2(d)}`; }

// Refresh the range button label (keeps the old function name for existing callers)
function syncDateInputs() {
  const s = DATA.labels[viewStart].slice(0, 10);
  const e = DATA.labels[viewEnd].slice(0, 10);
  document.getElementById('range-trigger').textContent =
    fmtDate(s + 'T00:00') + ' – ' + fmtDate(e + 'T00:00');
}

function toggleCal(e) {
  if (e) e.stopPropagation();
  const pop = document.getElementById('cal-pop');
  if (pop.hidden) openCal(); else pop.hidden = true;
}
function openCal() {
  selStart = DATA.labels[viewStart].slice(0, 10);
  selEnd   = DATA.labels[viewEnd].slice(0, 10);
  const p = selStart.split('-').map(Number);
  calY = p[0]; calM = p[1] - 1;
  document.getElementById('cal-hint').textContent = 'Tap start date';
  document.getElementById('cal-pop').hidden = false;
  renderCal();
}
...
function renderCal() {
  document.getElementById('cal-title').textContent = `${CAL_MONTHS[calM]} ${calY}`;
  const firstDow = new Date(calY, calM, 1).getDay();
  const dim = new Date(calY, calM + 1, 0).getDate();
  let h = ['S','M','T','W','T','F','S'].map(d => `<div class="cal-dow">${d}</div>`).join('');
  for (let i = 0; i < firstDow; i++) h += `<div class="cal-day blank"></div>`;
  for (let d = 1; d <= dim; d++) {
    const ds = ymd(calY, calM, d);
    if (ds < MIN_DATE || ds > MAX_DATE) { h += `<div class="cal-day disabled">${d}</div>`; continue; }
    let cls = 'cal-day';
    if (selStart && selEnd && ds >= selStart && ds <= selEnd) cls += ' in-range';
    if (ds === selStart || ds === selEnd) cls += ' sel';
    h += `<button type="button" class="${cls}" onclick="calPick(event,'${ds}')">${d}</button>`;
  }
  document.getElementById('cal-grid').innerHTML = h;
}
function calPick(e, ds) {
  if (e) e.stopPropagation();           // don't let the outside-click handler see this tap
  if (!selStart || selEnd) {            // start a fresh range
    selStart = ds; selEnd = null;
    document.getElementById('cal-hint').textContent = 'Tap end date';
    renderCal();
  } else if (ds < selStart) {           // earlier than start → move the start
    selStart = ds;
    renderCal();
  } else {                              // completes the range → apply and close
    selEnd = ds;
    applyRange(selStart, selEnd);
    document.getElementById('cal-pop').hidden = true;
  }
}
function calFull() {
  document.getElementById('cal-pop').hidden = true;
...
// Close the calendar when tapping anywhere outside it
document.addEventListener('click', e => {
  const pop = document.getElementById('cal-pop');
  if (pop.hidden) return;
  if (!pop.contains(e.target) && e.target.id !== 'range-trigger') pop.hidden = true;
});
...
function drawFloorPlan() {
  ...
  s += `<rect x="${ptacX}" y="${ptacY}" width="16" height="20" rx="2" fill="#A0B0C0" stroke="#6B7C93" stroke-width="1"/></g>`;
  ...
  s += `<rect x="${fuX}" y="${fuY}" width="18" height="20" rx="2" fill="#C4896A" stroke="#C4553A" stroke-width="1" stroke-dasharray="3,2"/></g>`;
  ...
  sensors.forEach(sen => {
    ...
    s += `<circle cx="${sen.x}" cy="${sen.y}" r="8" fill="${sen.color}" opacity="0.9"/></g>`;
  });
